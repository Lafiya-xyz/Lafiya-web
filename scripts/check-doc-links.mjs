...
for (const file of files) {
  ...
  for (const link of links) {
    ...
    // Links that escape the repo root point at sibling repos (e.g. `lafiya-docs`,
    // `lafiya-contracts`) in the wider org's multi-repo layout. This checker can
    // only validate links within this repo, so those are out of scope, not broken.
