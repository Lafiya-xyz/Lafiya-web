...
- `app/api/profile/photo/route.ts:56-61` — `.resize({ width: 800, height: 800, fit: "inside", withoutEnlargement: true })` runs unconditionally immediately after, which for `fit: "inside"` still requires a full decode of the source image regardless of how small the _output_ will be.
