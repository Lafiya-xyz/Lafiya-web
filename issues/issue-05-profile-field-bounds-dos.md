...
- Rate-limiting the public card page itself (a separate, complementary concern — this issue is specifically about bounding payload _size_, not request _rate_).
- The record-hash/attestation-lookup rate-limiting concern raised in the record-hash commitment-scheme issue in this batch.
- Any change to `lib/validation/profile.ts`'s existing limits (they are already adequate; the gap is that they are the _only_ enforcement).
