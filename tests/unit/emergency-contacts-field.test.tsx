...
// queryAllBy* (not getAllBy*) so an empty result is `[]` rather than a thrown
// error — the empty-state assertions below rely on that.
function nameInputs() {
  return screen.queryAllByPlaceholderText("Name") as HTMLInputElement[];
}
function phoneInputs() {
  return screen.queryAllByPlaceholderText(/^Phone/) as HTMLInputElement[];
}
function relationshipInputs() {
  return screen.queryAllByPlaceholderText("Relationship") as HTMLInputElement[];
}
function removeButtons() {
  return screen.queryAllByRole("button", {
    name: /remove contact/i,
...
describe("EmergencyContactsField — empty state", () => {
  it("renders a no-contacts message and an add-contact button, with no rows", () => {
    render(<EmergencyContactsField initialValues={[]} />);
    expect(nameInputs().length).toBe(0);
    expect(
      screen.getByText(/no emergency contacts added yet/i),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /add contact/i })).toBeInTheDocument();
  });

  it("has no remove buttons when there are no contacts", () => {
    render(<EmergencyContactsField initialValues={[]} />);
...
describe("EmergencyContactsField — removing contacts", () => {
  ...
  it("keeps remove enabled after reducing to one contact, allowing removal down to the empty state", () => {
    render(<EmergencyContactsField initialValues={[ALICE, BOB]} />);
    fireEvent.click(removeButtons()[0]);
